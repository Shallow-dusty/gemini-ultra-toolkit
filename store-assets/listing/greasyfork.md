...
```
Primer++ for Gemini™ is an unofficial, open-source companion userscript for Google Gemini (gemini.google.com). It adds modular quality-of-life features — daily message counter with weighted quota and reset-window framing, GitHub-style usage heatmap, conversation folders with drag-and-drop, local undo, and JSON import/export, JSON/CSV/Markdown/TXT/HTML/DOCX transcript export plus explicit transcript packets, prompt vault with one-click insert, prompt-chain queueing, and local delete undo, local message queue, local chat notes with JSON import/export and explicit local context packets, default-model selector, batch delete, quote reply, composer input counter, and assorted UI tweaks — all controllable from a draggable floating panel with four themes (Glass / Cyber / Paper / Auto).
