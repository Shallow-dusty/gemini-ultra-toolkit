...
const MAX_PROMPT_PACKET_ITEMS = 8;
const MAX_PROMPT_PACKET_CONTENT_LENGTH = 2400;
...
function formatPromptContextPacket(prompts, variables = {}, opts = {}) {
    const selected = normalizePromptList(Array.isArray(prompts) ? prompts : [prompts])
        .slice(0, MAX_PROMPT_PACKET_ITEMS);
    ...
    const sections = selected.map((prompt, index) => {
        ...
        const rendered = composePromptContent(prompt, variables).slice(0, MAX_PROMPT_PACKET_CONTENT_LENGTH);
        if (!rendered) return '';
        lines.push('Prompt:');
        lines.push(rendered);
        return lines.join('\n');
