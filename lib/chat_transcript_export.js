...
} = require('./transcript_fidelity.js');
...
function normalizeMessage(raw, index = 0) {
    ...
    const structure = normalizeMessageStructure(source.structure);
    if (!text && !structure) return null;
    const message = {
    ...
    if (structure) message.structure = structure;
    return message;
}
...
function normalizeTranscript(raw, opts = {}) {
    ...
    const canonical = source.format === CHAT_TRANSCRIPT_FORMAT &&
        source.schemaVersion === TRANSCRIPT_SCHEMA_VERSION;
    const transcript = {
        ...(canonical ? { format: CHAT_TRANSCRIPT_FORMAT, schemaVersion: TRANSCRIPT_SCHEMA_VERSION } : {}),
    ...
    const fidelity = normalizeFidelityReport(source.fidelity);
    if (fidelity) transcript.fidelity = fidelity;
    const metadata = normalizeTranscriptMetadata(source.metadata);
...
function exportTranscriptCSV(transcript, opts = {}) {
...
function exportBulkTranscriptCSV(bulkExport, opts = {}) {
