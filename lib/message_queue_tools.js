...
const DEFAULT_QUEUE_INTERVAL_MS = 1600;
const MIN_QUEUE_INTERVAL_MS = 800;
const MAX_QUEUE_INTERVAL_MS = 10000;
...
function createQueueRunToken(generation, context = {}) {
    return {
        generation,
        storageKey: cleanText(context.storageKey),
        routeKey: cleanText(context.routeKey)
    };
}

function isQueueRunCurrent(token, generation, context = {}) {
    if (!token || typeof token !== 'object') return false;
    return token.generation === generation
        && token.storageKey === cleanText(context.storageKey)
        && token.routeKey === cleanText(context.routeKey)
        && context.visible !== false
        && context.paused !== true;
}

function normalizeQueueIntervalMs(value) {
    const numeric = Number(value);
    if (!Number.isFinite(numeric)) return DEFAULT_QUEUE_INTERVAL_MS;
    return Math.max(MIN_QUEUE_INTERVAL_MS, Math.min(MAX_QUEUE_INTERVAL_MS, Math.round(numeric)));
}
...
function normalizeQueueData(raw, opts = {}) {
    ...
    return {
        ...
        intervalMs: normalizeQueueIntervalMs(source.intervalMs),
        items
    };
}
...
function setQueueInterval(data, intervalMs, opts = {}) {
    const state = normalizeQueueData(data, opts);
    state.intervalMs = normalizeQueueIntervalMs(intervalMs);
    return state;
}
